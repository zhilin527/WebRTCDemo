...
function SignalingChannel(sessionId) {
    ...
    for (var name in listeners)
        //defineProperty()直接在对象上定义一个新属性，或者修改一个存在的属性，返回这个对象
        Object.defineProperty(this, name, createEventListenerDescriptor(name, listeners));

    function createId() {
        return Math.random().toString(16).substr(2);//十六进制形式的字符串，substr(start,length)取子字符串
    };

    var es = new EventSource("/stoc/" + sessionId + "/" + userId);
    console.log('webrtc:: es.url -> ' + es.url);
    es.onerror = function () {
        es.close();
    };
